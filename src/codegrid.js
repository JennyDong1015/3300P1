...
(function (root, factory) {
  ...
  var gridPath = "codegrid-js-master/tiles/",
  ...
  Grid = function (tx, ty, zoom, json) {
    ...
    grid.getCode = function (lat, lng, callback) {
      ...
      if (!isInt(x) || !isInt(y) || x < 0 || y < 0 || x >= size || y >= size) {
  ...
  Zoomgrids = function (zlist) {
    ...
    function retrieveGrid(x, y, callback) {
      ...
      if (
        typeof jsonCache[cellx] !== "undefined" &&
      ...
      var tilePath =
        gridPath + cellx.toString() + "/" + celly.toString() + ".json";
    ...
    function handleJson(x, y, json, callback) {
      if (typeof json[x] !== "undefined" && typeof json[x][y] !== "undefined") {
